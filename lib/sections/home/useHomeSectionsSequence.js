...
export function useHomeSectionsSequence() {
   ...
   useEffect(() => {
      ...
      const context = gsap.context(() => {
         gsap.to(playhead, {
            ...
            snap: "frame",
            onUpdate: () => {
               const clampedFrame = Math.max(0, Math.min(frameTotal - 1, Math.round(playhead.frame)));
   ...
   const aboutStartFrame = Math.min(140, lastFrame);
   ...
   const overlayParallaxTransform = `translate3d(0, ${-30 * scrollProgress}px, 0)`;
